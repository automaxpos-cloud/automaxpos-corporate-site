...
If `https://automaxpos.com` serves another AutoMax application, restore the corporate site by re-running this repository's production workflow. The workflow uploads the corporate `dist/` contents to `/home/autosyyh/public_html` and verifies that `deployment.json` contains the deployed corporate commit.
